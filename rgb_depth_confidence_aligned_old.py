...
with device:
    ...
    while True:
        ...
        for queueName in queueEvents:
            ...
            # print(f"Printing the length of the packets at the given time: {len(packets)} for queue name: {queueName}", flush=True)
            if len(packets) > 0:
        ...
        if fps_counter == fps:
            end_time = time.time()
            print(f"Time taken to dump {fps} frames is: {round(end_time - start_time, 5)} seconds", flush=True)
            fps_counter = 0
            start_time = end_time
        ...
        if latestPacket["rgb"] is not None:
            ...
            print("rgb frame: " ,frameRgb.dtype, frameRgb.shape, flush=True)

            # cv2.imwrite(os.path.join("/home/harsha/Perception/oakd/data", "306_1512", "rgb", dt + ".png"), frameRgb)
        ...
        if latestPacket["disp"] is not None:
            ...
            print("disparity frame; ",frameDisp.dtype, frameDisp.shape, flush=True)
            # maxDisparity = stereo.initialConfig.getMaxDisparity()
            # # Optional, extend range 0..95 -> 0..255, for a better visualisation
            # frameDisp = (frameDisp * 65535. / maxDisparity).astype(np.uint16)
            # # Optional, apply false colorization
            # if 1: frameDisp = cv2.applyColorMap(frameDisp, cv2.COLORMAP_HOT)
            # frameDisp = np.ascontiguousarray(frameDisp)
            # cv2.imwrite(os.path.join("/home/harsha/Perception/oakd/data", "306_1512", "disp", dt + ".png"), frameDisp)
