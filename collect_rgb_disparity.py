...
clearTree = True
roomNumber = "306_" +str(datetime.now().strftime('%Y%m%d%H%M%S'))
...
with device:
    ...
    device.setIrLaserDotProjectorBrightness(350) # in mA, 0..1200
    device.setIrFloodLightBrightness(350)
...
#"disp",
        queueEvents = device.getQueueEvents(("rgb",  "disp", "synced_mono_left")) # to add mono, maybe IMU. # pull timestamp from here. a .get method for each packet.
        ...
        if latestPacket["disp"] is not None:
            #dt = str(latestPacket["disp"].getSequenceNum())
            frameDisp = latestPacket["disp"].getFrame()
            width, height = 1280, 720
            focal = width / (2 * math.tan(fov / 2 / 180 * math.pi))
            with np.errstate(divide='ignore'):
                depth = ((focal * baseline) / frameDisp).astype(np.uint16)#.astype(np.uint8) #.astype(np.uint16)
            # depth[depth == np.inf] = np.nan
            # print(f"Max value in depth: {np.nanmax(depth)}, min value: {np.nanmin(depth)}")
            depth = cv2.resize(depth, (640, 480))

            #frameDisp = cv2.resize(frameDisp, (640, 480))
            ...
            #frameDispDump = frameDisp.astype(np.uint16)
            ...
            cv2.imwrite(os.path.join("/home/harsha/Perception/oakd/data", roomNumber, "depth", dt + ".png"), depth)
